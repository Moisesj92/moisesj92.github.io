...
import { getDictionary, localePath, parsePathname, type Locale } from '@/i18n'
...
function useNav() {
  let { locale, path } = parsePathname(usePathname())
...
function NavItem({
  ...
  let isActive = usePathname().replace(/\/$/, '') === href
...
function ThemeToggle() {
  ...
  let otherTheme = resolvedTheme === 'dark' ? ('light' as const) : ('dark' as const)
  ...
  return (
    <button
      type="button"
      aria-label={mounted ? t.nav.themeSwitchTo[otherTheme] : t.nav.themeToggle}
...
function LocaleToggle() {
  let { locale, path, t } = useNav()
  let otherLocale: Locale = locale === 'es' ? 'en' : 'es'
  ...
  return (
    <Link
      ...
      className="flex h-10 items-center rounded-full bg-white/90 px-3 text-sm font-medium text-zinc-800 shadow-lg ring-1 shadow-zinc-800/5 ring-zinc-900/5 backdrop-blur-sm transition hover:text-teal-500 dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10 dark:hover:ring-white/20 dark:hover:text-teal-400"
    >
